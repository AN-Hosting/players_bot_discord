...
class SteamQueryProviderFactory {
    build() {
        ...
        return new SteamQueryProvider(process.env.GAME_TYPE, process.env.GAME_IP, parseInt(process.env.GAME_QUERY_PORT || '0'));
    }
}
class BattlemetricsProviderFactory {
    build() {
...
class CFToolsCloudProviderFactory {
    build() {
        ...
        return new CFToolsProvider(new CFToolsClientBuilder().withCache().withCacheConfiguration({
            gameServerDetails: 60,
        }).build(), process.env.CFTOOLS_HOSTNAME, parseInt(process.env.CFTOOLS_PORT));
    }
}
